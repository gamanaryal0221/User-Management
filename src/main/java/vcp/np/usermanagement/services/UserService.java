...
public class UserService {
    ...
    public Map<String, Object> saveUser(HttpServletRequest request) {
        ...
        try {
            ...
            String employerId = profile.getProperty(Constants.ADMIN_CLIENT_ID);
            Client employer = null;
            if (!Helper.isAdminClient(request.getSession(), employerId)) employerId = (String) request.getSession().getAttribute(Constants.Session.CLIENT_ID);
            ...
            Optional<Client> clientOptional = clientRepository.findById(Long.parseLong(employerId));
            employer = (clientOptional != null && clientOptional.isPresent())? clientOptional.get():null;
            if (employer == null) {
